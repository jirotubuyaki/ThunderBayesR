...
$\Sigma_{k}$ is a variance-covariance matrix of $k$th cluster. $i$ and $j$ are rows and columns' number of $\Sigma_{k}$. First several durations of iterations which are called as "burn in" are error ranges. For that reason, "burn in" durations are abandoned.
